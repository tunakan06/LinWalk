...
//					Debug.LogWarning( command.ToErrorString("Not Skippable Wait type" + command.WaitType +" is added"));
		...
		//全エフェクトを強制スキップして終了する
		public void ForceSkipAllEffect(bool ignoreLoopEffect)
		{
			ForceSkipCommandList(notWaitCommandList,ignoreLoopEffect);
			ForceSkipCommandList(waitCommandList,ignoreLoopEffect);
		}
		
		//NoWait系のエフェクトをすべて強制スキップして終了する
		public void ForceSkipNoWaitEffect(bool ignoreLoopEffect)
		{
			ForceSkipCommandList(notWaitCommandList,ignoreLoopEffect);
		}

		void ForceSkipCommandList(List<AdvCommandWaitBase> list, bool ignoreLoopEffect )
		{
			...
			foreach (AdvCommandWaitBase command in tmp)
			{
				if (command is IAdvCommandEffect skip)
				{
					if (ignoreLoopEffect && skip is IAdvCommandEffectLoop loop && loop.IsLoopEffect())
					{
						continue;
					}
					skip.OnEffectSkip();
